        ...
        <form action="{{ route('courses.update', $course->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <input type="text"
                    ...
                    is-invalid
                @enderror"
                    id="title" value="{{ old('title',$course->title ) }}" name="title">
                @error('title')
            ...
            <div class="mb-3">
                ...
                <input type="file" class="form-control @error('image') is-invalid @enderror" id="image" accept=".png,.jpg" name="image" >
                 @if(isset($course->image))
                    <img width="100px" src="{{ asset($course->image) }}" alt="{{ $course->title }}">
                @endif
            ...
            <div class="mb-3">
                ...
                <input type="number" class="form-control @error('hour') is-invalid @enderror"
                    value="{{ old('hours',$course->hours ) }}" id="hour" name="hour">
                @error('hour')
                    <small class="invalid-feedback">{{ $message }}</small>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control @error('content') is-invalid @enderror" id="content" name="content" rows="4">{{ $course->content }}</textarea>
                @error('content')
                    <small class="invalid-feedback">{{ $message }}</small>
                @enderror
            </div>
